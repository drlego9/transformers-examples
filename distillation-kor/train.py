...
MODEL_CLASSES = {
    ...
    'kobert': (BertConfig, BertForMaskedLM, BertTokenizer),
...
def sanity_checks(args):
    ...
    if args.mlm:
        ...
        assert (args.student_type in ['roberta', 'distilbert', 'distilkobert'])
        assert (args.teacher_type in ['roberta', 'bert', 'kobert'])
    else:
        assert (args.student_type in ['gpt2']) and (args.teacher_type in ['gpt2'])

    if args.teacher_type != args.student_type:
        assert (args.student_type == 'distilbert' and args.teacher_type == 'bert') or \
            (args.student_type == 'distilkobert' and args.teacher_type == 'kobert')
...
def main():
    ...
    parser.add_argument("--student_type", type=str, required=True,
                        choices=["distilbert", "roberta", "gpt2", "distilkobert"],
    ...
    parser.add_argument("--teacher_type", choices=["bert", "roberta", "gpt2", "kobert"], required=True,
                        help="Teacher type (BERT, RoBERTa, GPT2, KoBERT).")
    ...
    if args.is_master:
        ...
        # git_log(args.dump_path)  # FIXME

    student_config_class, student_model_class, _ = MODEL_CLASSES[args.student_type]
    teacher_config_class, teacher_model_class, teacher_tokenizer_class = MODEL_CLASSES[args.teacher_type]

    ### TOKENIZER ###
    if args.teacher_type == 'kobert':
        kobert_tokenizer_configs = json.load(open('.kobert/kobert_tokenizer_config.json', 'r'))
        tokenizer = teacher_tokenizer_class(**kobert_tokenizer_configs)
        tokenizer.max_model_input_sizes[args.teacher_name] = 512
    else:
    ...
    ## STUDENT ## TODO: check student configuration
    logger.info(f'Loading student config from {args.student_config}')
    stu_architecture_config = student_config_class.from_pretrained(args.student_config)
    stu_architecture_config.output_hidden_states = True

    if args.student_pretrained_weights is not None:
        # `student_pretrained_weights` are one of the outputs of `python extract_distilbert.py`
    ...
    ## TEACHER ##
    if args.teacher_type == 'kobert':
        # TODO: make the pretrained directory an argparse argument
        teacher = teacher_model_class.from_pretrained('.kobert/pretrained/', output_hidden_states=True)
    else:
